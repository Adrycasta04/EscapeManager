...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        layer/.style={draw, rectangle, minimum width=0.98\textwidth, minimum height=2.3cm, fill=gray!5, dashed, font=\bfseries\sffamily, rounded corners},
        llabel/.style={anchor=north west, font=\bfseries\sffamily\scriptsize, gray!60, xshift=4mm, yshift=-2mm},
        comp/.style={draw, rectangle, fill=white, minimum width=3.8cm, minimum height=1.1cm, align=center, font=\small\sffamily, thick},
        boundary/.style={comp, fill=green!10},
        control/.style={comp, fill=yellow!15},
        entity/.style={comp, fill=red!10},
        dao/.style={comp, fill=blue!5},
        arrow/.style={-Stealth, thick}
    ]

        % Layer 1: Boundary
        \node[layer] (L1) {};
        \node[llabel] at (L1.north west) {Presentation Layer (Boundary)};
        \node[boundary, xshift=-2.4cm, yshift=-2mm] at (L1.center) (cli) {\guillemotleft boundary\guillemotright\\CLI Interface};
        \node[boundary, xshift=2.4cm, yshift=-2mm] at (L1.center) (mock) {\guillemotleft boundary\guillemotright\\Web/UI Mockups};

        % Layer 2: Control
        \node[layer, below=1cm of L1] (L2) {};
        \node[llabel] at (L2.north west) {Application Layer (Control)};
        \node[control, yshift=-2mm] at (L2.center) (ctrl) {\guillemotleft control\guillemotright\\Business Controllers};

        % Layer 3: Entity
        \node[layer, below=1cm of L2] (L3) {};
        \node[llabel] at (L3.north west) {Domain Layer (Entity)};
        \node[entity, yshift=-2mm] at (L3.center) (ent) {\guillemotleft entity\guillemotright\\Domain Model Objects};

        % Layer 4: Persistence
        \node[layer, below=1cm of L3] (L4) {};
        \node[llabel] at (L4.north west) {Data Source Layer (Persistence)};
        \node[dao, yshift=-2mm] at (L4.center) (dao) {DAO System / DB\\(PostgreSQL Persistence)};

        % Connections
        \draw[arrow] (cli.south) -- (ctrl.north -| cli.south);
        \draw[arrow] (mock.south) -- (ctrl.north -| mock.south);
        \draw[arrow] (ctrl) -- (ent);
        \draw[arrow] (ent) -- (dao);

    \end{tikzpicture}
    \caption{Architettura a strati e separazione BCE del sistema EscapeManager (UML Component Diagram)}
