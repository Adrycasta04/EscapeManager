...
Partendo dai casi d'uso del Capitolo 2, la progettazione segue i principi dell'Object-Oriented Design (OOD) con architettura BCE (Boundary-Control-Entity). In questo capitolo viene presentata la struttura statica del sistema e l'applicazione dei Design Pattern scelti per mantenere il codice estensibile e disaccoppiato.

\section{Architettura a Strati e BCE}
L'architettura è organizzata in strati coerenti con il paradigma BCE. L'interfaccia (CLI/mockup) comunica solo coi Controller, i quali gestiscono le entità del dominio e delegano la persistenza al layer DAO. Le dipendenze vanno solo verso il basso.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/fig_3_1_component_diagram.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/fig_3_2_package_diagram.png}
...
\begin{itemize}
    ...
    \item \textbf{Single Table Inheritance} per la gerarchia \texttt{Utente}: semplifica le query e l'autenticazione, assumendosi il lieve svantaggio di tollerare colonne vuote (\texttt{NULL}) per gli attributi non in comune.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/fig_3_3_domain_model.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/fig_3_4_class_diagram_controllers.png}
...
Per scollegare i Controller dal database concreto, il sistema usa il pattern \textbf{Abstract Factory} (DAO Factory): la Factory espone metodi per ottenere i DAO di tutte le entità (Stanza, Prenotazione, WaitingList), e i Controller lavorano solo con le interfacce restituite, senza sapere quale database ci sia sotto. La Figura \ref{fig:class_diagram_dao} mostra il package \texttt{dao}, con le interfacce, le classi concrete e la connessione unica (Singleton).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig_3_5_class_diagram_dao.png}
    \caption{Class Diagram: Package DAO (Abstract Factory)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig_3_6_object_diagram.png}
...
L'Object Diagram mostra anche come i pattern State e Strategy coesistano a runtime sulla stessa istanza di Stanza: lo stato corrente governa le transizioni, la strategia di prezzo governa il calcolo economico, e la WaitingList gestisce le notifiche.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/fig_3_7_state_machine_room.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/fig_3_8_er_diagram.png}
...
La Figura \ref{fig:sequence_prenotazione} mostra il Sequence Diagram relativo al caso d'uso principale. Il flusso segue il pattern BCE: la CLI (Boundary) delega al \texttt{PrenotazioneController} (Control), che recupera i dati attraverso la DAO Factory e demanda il calcolo del prezzo all'entità \texttt{Stanza} (Entity). Il Controller non accede mai direttamente al database.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/fig_3_9_sequence_prenotazione.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig_3_10_sequence_tariffe.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/fig_3_11_sequence_gestione_stato.png}
