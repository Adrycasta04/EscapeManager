...
\section{Template Method Pattern (GenericDAO)}
L'interfaccia \texttt{GenericDAO<T, ID>} definisce un ``contratto'' CRUD uniforme per tutte le entità. Sottointerfacce come \texttt{StanzaDAO} o \texttt{PrenotazioneDAO} aggiungono operazioni di dominio. Infine, le implementazioni concrete (es. \texttt{StanzaDAOPostgres}) forniscono le query SQL. Questo approccio ricalca il \textbf{Template Method Pattern} (GoF --- Comportamentale): l'interfaccia definisce lo ``scheletro algoritmico'', le classi implementano i passi specifici.

\begin{lstlisting}[language=Java, caption=Template Method Pattern: GenericDAO e specializzazione]
public interface GenericDAO<T, ID> {
    void save(T entity);
    T findById(ID id);
    List<T> findAll();
...
\begin{itemize}
    ...
    \item \textbf{UC3:} mostra il flusso di aggiornamento stato stanza (Figura \ref{fig:sequence_gestione_stato}), con delega allo State Pattern e il flusso alternativo per transizioni non valide.
...
\begin{figure}[H]
    ...
    \label{fig:sequence_gestione_stato}
\end{figure}
