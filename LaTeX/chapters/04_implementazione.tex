...
Questo capitolo descrive l'implementazione del modello di dominio e dello schema del database in codice Java. Il progetto adotta un'architettura a strati basata sul modello BCE, pensata per separare le responsabilità e facilitare i test.

\section{Architettura dei Package}

L'architettura del sistema è divisa in quattro package (Figura \ref{fig:package_diagram}), con dipendenze che vanno sempre nella stessa direzione: la CLI comunica solo coi controller, i controller usano il dominio e i DAO, e il DAO è l'unico che parla col database.
...
La classe \texttt{Prenotazione} ha molti attributi, e costruirla con un costruttore tradizionale sarebbe fragile e poco leggibile. Per questo l'istanziazione avviene tramite il pattern \textbf{Builder}:
