...
public class DisponibileState implements RoomState {
    ...
    public void terminaSessione(Stanza stanza) {
        throw new IllegalStateException("Impossibile terminare una sessione non avviata.");
    }
    // ... altri metodi
}
\end{lstlisting}
...
Il calcolo del prezzo della prenotazione (UC2) è demandato alla \texttt{PricingStrategy}. Il Controller, in base alla data della prenotazione, inietta la strategia corretta all'interno dell'entità.
...
La lista d'attesa (UC3) implementa un sistema Publisher-Subscriber puro in Java, disaccoppiando l'evento di cancellazione di una prenotazione dall'invio effettivo delle notifiche.
...
public class WaitingList {
    ...
    public void notifyObservers(Stanza stanza, LocalDateTime slot) {
        for (WaitingListObserver obs : observers) {
            obs.update(stanza, slot); // Notifica asincrona
        }
    }
}
\end{lstlisting}