...
L'architettura del sistema è strutturata in quattro package distinti (Figura \ref{fig:package_diagram}), le cui interdipendenze seguono un vincolo di direzionalità rigorosa. Tale approccio garantisce un elevato disaccoppiamento: il layer di presentazione (\texttt{cli}), agendo come Boundary, non ha visibilità diretta della persistenza, ma interagisce esclusivamente con i \texttt{controllers} (layer Control). Questi ultimi orchestrano il flusso dei dati interfacciandosi con il \texttt{domain} (Entity) e delegando l'accesso fisico al database al package \texttt{dao} (Persistence).
...
Per non mischiare la logica dell'applicazione con il codice SQL, l'accesso a PostgreSQL è isolato tramite il pattern \textbf{Data Access Object (DAO)}, garantendo che la business logic rimanga agnostica rispetto alla tecnologia di persistenza.
...
Data la presenza di molteplici attributi nella classe \texttt{Prenotazione}, l'istanziazione è delegata al pattern creazionale \textbf{Builder}, ottimizzando la leggibilità e prevenendo errori nell'ordine di inizializzazione dei parametri.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/fig_4_1_class_diagram_exceptions.png}
...
Per mantenere l'integrità dei dati e rispettare il principio di Information Hiding, l'accesso alla lista interna degli \texttt{Observer} nella classe \texttt{WaitingList} avviene tramite copia difensiva:
